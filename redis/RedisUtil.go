...
//初始化redis
func InitRedis() {
	InitRedisUtilClient(config.Conf.Redis.Addr, config.Conf.Redis.Port, config.Conf.Redis.Password)
}

func InitRedisUtilClient(address string, port int, pwd string) (*RedisUtil, error) {
	//连接redis
